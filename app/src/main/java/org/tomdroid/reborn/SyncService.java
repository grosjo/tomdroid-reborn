...
public abstract class SyncService {
		...
		if (!(localGuids == null || localGuids.getCount() == 0)) {
			...
			do {
				...
				if(!remoteGuids.contains(localGuid)) {
					...
					TTime syncDate = new TTime();
					syncDate.parseTomboy(syncDateString);
					int compareSync = TTime.compare(syncDate, note.getLastChangeDate());
	...
	// deal with notes in both - compare and push, pull or diff
		...
		// if different guids, means conflicting titles
			...
			int compareSyncLocal = TTime.compare(syncDate, localNote.getLastChangeDate());
			int compareSyncRemote = TTime.compare(syncDate, remoteNote.getLastChangeDate());
			int compareBoth = TTime.compare(localNote.getLastChangeDate(), remoteNote.getLastChangeDate());
