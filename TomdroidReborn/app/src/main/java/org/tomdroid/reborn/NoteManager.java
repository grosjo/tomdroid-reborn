...
public class NoteManager {
	...
	public static Uri getUriByGuid (Activity activity,  String guid) {
		Uri uri = Uri.parse(TPrefs.CONTENT_URI+"/"+getNoteIdByGUID(activity, guid));
		return uri;
	}
	...
	public static Uri putNote(Activity activity, Note note) {
		...
		if (managedCursor == null || managedCursor.getCount() == 0) {
			...
			uri = Uri.parse(TPrefs.CONTENT_URI+"/"+getNoteIdByGUID(activity, note.getGuid()));
	...
	{
		Uri uri = Uri.parse(TPrefs.CONTENT_URI+"/"+id);
